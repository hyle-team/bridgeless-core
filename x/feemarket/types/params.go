...
func (p Params) Validate() error {
	...
	if p.BaseFee.IsNegative() {
		return errorsmod.Wrapf(errors.New("invalid base fee"), "initial base fee cannot be negative: %s", p.BaseFee)
	}
...
func validateMinGasMultiplier(i interface{}) error {
	...
	if v.IsNil() {
		return errorsmod.Wrap(errors.New("invalid gas multiplier"), "invalid parameter: nil")
	}

	if v.IsNegative() {
		return errorsmod.Wrapf(errors.New("invalid gas multiplier"), "value cannot be negative: %s", v)
	}

	if v.GT(sdk.OneDec()) {
		return errorsmod.Wrapf(errors.New("invalid gas multiplier"), "value cannot be greater than 1: %s", v)
	}
	return nil
}
