...
# Set client config
bridgeless-cored config keyring-backend "$KEYRING"
bridgeless-cored config chain-id "$CHAINID"

# if $KEY exists it should be deleted
bridgeless-cored keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
bridgeless-cored init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(bridgeless-cored keys list | grep  "address: " | cut -c12-)
...
bridgeless-cored add-genesis-account $KEY 100000000000000000000000000aevmos --keyring-backend $KEYRING
...
bridgeless-cored gentx $KEY 1000000000000000000000aevmos --keyring-backend $KEYRING --chain-id "$CHAINID"
...
# Collect genesis tx
bridgeless-cored collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
bridgeless-cored validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
bridgeless-cored start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.api eth,txpool,personal,net,debug,web3
